...
@else
    <div id="footer" >
        <table style="width: 100%; color:grey;">
            <tr>
                <td colspan="4" style="color:#0096D6; font-size: 8pt;">{{ checkIfEmpty('11', $ideaA) }} • {{ checkIfEmpty('12', $ideaA) }}</td>
            </tr>
            <tr style="font-size: 5pt;">
                <td colspan="4"><strong>{{ checkIfEmpty('1', $ideaA) }}</strong></td>
            </tr>
            <tr style="font-size: 5pt;">
                <Td>{{ checkIfEmpty('2', $ideaA) }}</td>
                <Td>T: {{ checkIfEmpty('5', $ideaA) }}</td>
                <Td colspan="2">Sąd Rejonowy dla Wrocławia-Fabrycznej, VI Wydział Gospodarczy Krajowego Rejestru Sądowego </td>
            </tr>
            <tr style="font-size: 5pt;">
                <Td width="75px">{{ checkIfEmpty('3', $ideaA) }} {{ checkIfEmpty('13', $ideaA) }}</td>
                <Td width="85px">F: {{ checkIfEmpty('6', $ideaA) }}</td>
                <Td >KRS {{ checkIfEmpty('7', $ideaA) }}, NIP: {{ checkIfEmpty('8', $ideaA) }}, wysokość kapitału zakładowego {{ checkIfEmpty('9', $ideaA) }} wpłacony w całości</td>
                <td width="35px"><div class="page-number"></div></td>
            </tr>
