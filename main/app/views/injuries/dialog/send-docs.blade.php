...
<div class="modal-body" style="overflow:hidden;">
    <form action="{{ URL::to('injuries/docs/send', array($injury->id)) }}" method="post"  id="dialog-form-mail">
        {{Form::token()}}
        <div class="row">
            <div class="col-sm-12 col-lg-6">
                ...
                <ul class="list-group">
                    ...
                    @endif
                    @if($driver)
                    <li class="list-group-item @if(!$driver->email || $driver->email == '') list-group-item-danger @endif">
                        <div class="checkbox">
                            <label>
                                <input name="addressees['driver']" value="{{ $driver->email }}" type="checkbox" @if(!$driver->email || $driver->email == '') disabled @endif> Kierowca - {{ $driver->name }} - {{ $driver->email }}
                            </label>
                        </div>
                    </li>
                    @endif
                    @if($branch && isset($branch_emails[0]) && count($branch_emails[0]) > 0)
                        @foreach($branch_emails[0] as $branch_email)
                        <li class="list-group-item">
                            <div class="checkbox">
                                <label>
                                    <input name="branches[]" value="{{ $branch_email }}" type="checkbox">
                                    Warsztat - {{ $branch->short_name }} - {{ $branch_email }}
                                </label>
                            </div>
                        </li>
                        @endforeach
            ...
            <div class="col-sm-12 col-lg-6">
                ...
                <div class="form-group">
                    <label>Wybór wzoru:</label>
                    <select id="template_name" name="template_name" form="dialog-form-mail">
                        <option value="injury-docs">Dział Likwidacji Szkód</option>
                        <option value="billing-department">Dział rozliczeń</option>
                    </select>
                </div>
            </div>
