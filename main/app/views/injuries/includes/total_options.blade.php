<td>
    @if(Auth::user()->can('zlecenia_(szkody)#zarzadzaj'))
      ...
      <ul class="dropdown-menu" role="menu">
        <li>
          <a href="#" target="{{ URL::route('injuries-getTotalFinished', array($injury->id)) }}" class="modal-open-sm" data-toggle="modal" data-target="#modal-sm"
             @if($injury->locked_status == 5)
             disabled="disabled"
                  @endif
        ...
        <li>
          <a href="#" target="{{ URL::route('injuries-getRestore', array($injury->id)) }}" class="modal-open-sm" data-toggle="modal" data-target="#modal-sm">przywróć szkodę</a>
        </li>
        <li class="divider"></li>
          @if($injury->step == 30)
              <li>
          ...
          @endif
          <li>
              <a href="#" target="{{ URL::route('injuries-getResignationClaims', array($injury->id)) }}" class="modal-open-sm" data-toggle="modal" data-target="#modal-sm">całkowita rezygnacja z roszczeń</a>
          </li>
          <li>
