...
Zlecenia (szkody) - zakończone totalnie
</span>
	...
	<div  id="injuries-container">
		...
		<table class="table  table-hover  table-condensed" id="users-table">
			...
			<?php
				...
				>
					<td>{{$lp++}}.</td>
					<Td>@include('injuries.includes.total-finished_statuses')</td>
					<td>
						@if($injury->status)
					...
					<td>
						...
						@elseif($injury->theftStatus && in_array($injury->step, [40, 41, 42, 43, 44, 45, 46]))
							<span class="label label-border-info status-btn tips" title="{{ $injury->theftStatus->name }}">{{ $injury->theftStatus->name }}</span>
						@else
							<span class="label label-border-info status-btn tips" title="{{ $injury->status->name }}">{{ $injury->status->name }}</span>
						@endif
					</td>
                    ...
                    @include('injuries.includes.total-finished_options')
				</tr>
				<?php }
			 ?>
