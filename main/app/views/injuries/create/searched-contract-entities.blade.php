@foreach($contracts as $contract)
    @foreach($contract->vehicles as $vehicle)
        <?php
                $policy = null;
                if(isset($vehicle->contract_internal_agreements[0]))
                {
                    foreach($vehicle->contract_internal_agreements[0]->policies as $c_policy)
                    {
                        if($c_policy->policy_date_from && $c_policy->policy_date_to && stripos($c_policy->policy_risks,'AC') !== false &&\Carbon\Carbon::createFromFormat('Y-m-d', $c_policy->policy_date_from) -> lte(\Carbon\Carbon::now()) && \Carbon\Carbon::createFromFormat('Y-m-d', $c_policy->policy_date_to) -> gte(\Carbon\Carbon::now()) )
                            {
                                $policy = $c_policy;
                            }
                    }
        ...
        <tr>
            ...
            <td>
                @if($contract->salesProgram)
                    {{ $contract->salesProgram->name }}
                    <br>
                    <span class="label @if($contract->verified_sales_program) label-info @else label-danger @endif">
                        {{ $contract->salesProgram->name_key }}
                    </span>
                @else
                    ---
                @endif
            ...
            <td>
                @if(contractStatus($contract->contract_status) == 0) 
            ...
            <td>
                @if(isset($vehicle->contract_internal_agreements[0]))
                    @if($vehicle->contract_internal_agreements[0]->active == 'Nieaktywna')
                        <span class="label label-danger">{{ $vehicle->contract_internal_agreements[0]->active }}</span><br >
                    @else
                        {{ $vehicle->contract_internal_agreements[0]->active }}<br >
                    @endif
                    {{ $vehicle->disposal }}
                @endif
            </td>
            ...
            <td>
                @if($contract->verified_sales_program)
                    <span class="btn btn-xs btn-success suggest-branch"
                          data-contract="{{ $contract->id }}"
                          data-vehicle="{{ $vehicle->id }}"
                          data-agreement="{{ isset($vehicle->contract_internal_agreements[0]) ? $vehicle->contract_internal_agreements[0]->id : null }}"
                          data-type="vehicle"
                          @if($policy)
                            data-policy="{{ $policy->policy_id }}"
                          @else
                            data-policy=""
                          @endif
                    >
                        <i class="fa fa-th-list fa-fw"></i> zaproponuj serwis
                    </span>
                @else
            ...
            <td>
                <form action="{{ url('injuries/make/create-new-entity') }}" method="post">
                    ...
                    @if($policy)
                        {{ Form::hidden('policy_id', $policy->policy_id) }}
                    @endif
            ...
            <td colspan="13">
                <div class="row">
                   <div class="col-sm-12 col-md-10 col-md-offset-1 col-lg-6 col-lg-offset-3 text-center">
                       @if($policy)
                       <div class="btn btn-xs btn-primary text-center" data-toggle="collapse" data-target="#collapsePolicies{{$vehicle->id}}{{$contract->id}}" aria-expanded="false" aria-controls="collapsePolicies{{$vehicle->id}}{{$contract->id}}">
                       ...
                       <div class="btn btn-xs btn-warning text-center" data-toggle="collapse" data-target="#collapseInjuries{{$vehicle->id}}{{$contract->id}}" aria-expanded="false" aria-controls="collapseInjuries{{$vehicle->id}}{{$contract->id}}">
                           <i class="fa fa-arrow-down fa-fw" aria-hidden="true"></i>
                           szkody na przedmiocie <span class="badge">{{ count( isset($vehicle->injuries) ? $vehicle->injuries : [] ) }}</span>
                       </div>

                       <div class="btn btn-xs btn-warning text-center" data-toggle="collapse" data-target="#collapseUnprocessed{{$vehicle->id}}{{$contract->id}}" aria-expanded="false" aria-controls="collapseUnprocessed{{$vehicle->id}}{{$contract->id}}">
                           <i class="fa fa-arrow-down fa-fw" aria-hidden="true"></i>
                           szkody nieprzetworzone <span class="badge">{{ count( isset($vehicle->unprocessed) ? $vehicle->unprocessed : [] ) }}</span>
                       </div>

                       <div class="btn btn-xs btn-info text-center" data-toggle="collapse" data-target="#collapseLetters{{$vehicle->id}}{{$contract->id}}" aria-expanded="false" aria-controls="collapseLetters{{$vehicle->id}}{{$contract->id}}">
                           <i class="fa fa-arrow-down fa-fw" aria-hidden="true"></i>
                           pisma dopasowane do przedmiotu <span class="badge">{{ count(  isset($vehicle->letters) ? $vehicle->letters : [] ) }}</span>
                       </div>
                   </div>
                    @if($policy)
                    <div class="col-sm-12 col-lg-8 col-lg-offset-2 collapse" id="collapsePolicies{{$vehicle->id}}{{$contract->id}}">
                    ...
                    <div class="col-sm-12 collapse" id="collapseInjuries{{ $vehicle->id }}{{$contract->id}}">
                            ...
                            <table class="table table-hover table-condensed">
                                ...
                                @foreach($vehicle->injuries as $k => $injury)
                                    <tr>
                                        ...
                                        <td>
                                            @if($contract->contract_number == $injury->vehicle->nr_contract)
                    ...
                    @if(isset($vehicle->unprocessed))
                        <div class="col-sm-12 collapse" id="collapseUnprocessed{{ $vehicle->id }}{{$contract->id}}">
                            <div class="panel panel-warning marg-top-min">
                                <table class="table  table-hover  table-condensed">
                                    <thead class="bg-warning">
                                    <Th style="width:30px;">lp.</th>
                    ...
                    <div class="col-sm-12 col-lg-8 col-lg-offset-2 collapse" id="collapseLetters{{ $vehicle->id }}{{$contract->id}}">
