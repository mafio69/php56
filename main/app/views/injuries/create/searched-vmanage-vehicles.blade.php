<table class="table table-condensed table-contracts">
    ...
    @foreach($vehicles as $k => $vehicle)
        <tr @if(isset($vehicle['is_trashed']) && $vehicle['is_trashed']) class="bg-danger" @endif>
            <td>
                {{ ++ $k  }}.
            </td>
            <td>
            ...
            <td>
                @if($vehicle['sales_program'])
                    {{ $vehicle['sales_program']->name }}
                    <br>
                    <span class="label label-info">
                        {{ $vehicle['sales_program']->name_key }}
                    </span>
                @else
                    ---
                @endif
            ...
            <td>
                @if($vehicle['sales_program'] && Plan::where('sales_program', $vehicle['sales_program']->name_key)->first())
                    <span class="btn btn-xs btn-success suggest-branch" data-vehicle="{{ $vehicle['id'] }}" data-type="vmanage">
            ...
            <td>
                <form action="{{ url('injuries/make/create-new-entity') }}" method="post">
                    {{ Form::token() }}
                    {{ Form::hidden('vmanage_vehicle_id', $vehicle['id']) }}
