            ...
            >
                ...
                <Td>
                    ...
                    @if($leasingAgreement->has_yacht == 1)
                        <i class="fa fa-ship blue" title="umowa z jachtem"></i>
                    @endif
                    @if(!is_null($leasingAgreement->reported_to_resume))
                        <i class="fa fa-flag warning tips"
                           title="umowa oznaczona do wznowienia {{ $leasingAgreement->reported_to_resume }}"></i>
                    @endif
                ...
                <td>
                    @if(Auth::user()->can('kartoteka_polisy#wejscie'))
                        <a type="button" class="btn btn-link"
                           href="{{ URL::to('insurances/info/show', [$leasingAgreement->id]) }}">
                ...
                <td>{{ ($leasingAgreement->insurances->last()->insuranceCompany)
                ?$leasingAgreement->insurances->last()->insuranceCompany->name:'---'}}</td>
