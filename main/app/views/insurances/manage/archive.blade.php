...
@section('table-content')
    <table class="table table-hover  table-condensed" id="users-table">
        ...
        <th>polisa od</th>
        <th>polisa do</th>
        <Th>data przeniesienia do archiwum</Th>
        <th>SU [netto]</th>
        <th>nr zgłoszenia</th>
        <th></th>
        <th></th>
        ...
        @foreach ($leasingAgreements as $leasingAgreement)
            ...
            @if(Session::has('last_agreement') && $leasingAgreement->id == Session::get('last_agreement'))
                ...
                <td>{{ ($leasingAgreement->insurances->last() && $leasingAgreement->insurances->last()->insuranceCompany)?$leasingAgreement->insurances->last()->insuranceCompany->name:'---'}}</td>
                <td>{{ ($leasingAgreement->insurances->last()) ? $leasingAgreement->insurances->last()->date_from : '' }}</td>
                <td>{{ ($leasingAgreement->insurances->last()) ? $leasingAgreement->insurances->last()->date_to : '' }}</td>
                <td>{{ substr($leasingAgreement->archive, 0, -3) }}</td>
                ...
                <td>
                    @include('insurances.manage.options.archive')
                </td>
                <td>
