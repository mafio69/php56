            ...
            <table class="table table-hover" >
                @foreach($agreement->files as $k => $v)
                    <tr class="vertical-middle">
                        ...
                        <td>
                            @if($v->type == 3)
                              <a href="{{ URL::to('insurances/documents/download-generate-doc', [$v->id]) }}" target="_blank" class="fa fa-floppy-o blue pointer md-ico"></a>
                            @else
                              <a href="{{ URL::to('insurances/manage-actions/download-document', [$v->id]) }}" target="_blank" class="fa fa-floppy-o blue pointer md-ico"></a>
                            @endif
                        </td>
                        <td>
                            @if($v->type == 3)
                        ...
                        <td>
                            @if($v->type == 3)
                              @if($v->category_object()->first())
                                 {{$v->category_object()->first()->name}}
                              @endif
                            @else
                        ...
                        <Td>
                            @if(Auth::user()->can('kartoteka_polisy#zarzadzaj'))
                                <button type="button" class="btn btn-danger modal-open-sm" target="{{ URL::to('insurances/info-dialog/delete-document', [$v->id]) }}"  data-toggle="modal" data-target="#modal-sm">usuń</button>
                            @endif
                        </td>
                    </tr>
                @endforeach
