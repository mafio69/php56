...
class HestiaReport implements ReportsInterface{
    ...
    public function generateReport()
    {
        ...
        $template = \Config::get('webconfig.WEBCONFIG_UPLOADS_FOLDER') . '/templates/HESTIA_reinvoice_report.xlsx';
    ...
    private function generateNewSheet($excel)
    {
        ...
        $newSheet->getStyle('A17:U'.$row)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER)->setWrapText(true);
        $newSheet->getStyle('A17:U'.($row-1))->applyFromArray($this->borderStyleSmall);
        ...
        $newSheet->getStyle('A'.$startingRow.':U'.$row)->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER)->setWrapText(true);
        $newSheet->getStyle('A'.$startingRow.':U'.($row-1))->applyFromArray($this->borderStyleSmall);
    }
